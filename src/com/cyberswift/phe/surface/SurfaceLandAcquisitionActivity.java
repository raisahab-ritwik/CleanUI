...
public class SurfaceLandAcquisitionActivity extends Activity implements OnCheckedChangeListener, OnClickListener, ServerResponseCallback, LocationListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContext = SurfaceLandAcquisitionActivity.this;
		initView();
	...
	public void onPostClicked(View v) {
		...
		if (Util.checkConnectivity(mContext) && isInputValid) {
			...
			if (isInsert) {
				...
				volleyTaskManager.doPostSurfaceInsertLandAcquisition(requestMap);

			} else if (isUpdate) {

				isUpdateService = true;
				Log.v("SurfaceLand", ">>----------IS UPDATE >>------------");
				requestMap.put("ID", user.getSurfaceLandAcquisitionUpdateId());
				volleyTaskManager.doPostSurfaceUpdateLandAcquisition(requestMap);
		...
		if (isInsertService) {
			...
			if (resultJsonObject.toString() != null && !resultJsonObject.toString().trim().isEmpty()) {
				...
				try {
					...
					String result = new JSONObject(resultJsonObject.optString("SaveSurfaceLandAcquisitionResult")).optString("RES");
			...
			if (resultJsonObject.toString() != null && !resultJsonObject.toString().trim().isEmpty()) {
				...
				try {
					...
					String result = new JSONObject(resultJsonObject.optString("UpdateSurfaceLandAcquisitionResult")).optString("RES");
	...
	private void parseInitialFetchedData(JSONObject resultJsonObject) {
		...
		try {
			...
			JSONArray resultJSONArray = new JSONArray(resultJsonObject.optString("GetSurfaceLandAcquisitionResult"));
			...
			if (resultJSONArray.length() > 0) {
				...
				for (int i = 0; i < resultJSONArray.length(); i++) {
					...
					UserClass user = Util.fetchUserClass(mContext);
					user.setSurfaceLandAcquisitionUpdateId(landAcquisition.getID());
					Util.saveUserClass(mContext, user);
	...
	private boolean checkFormDataInput() {
		...
		if (tv_latitude.getText().toString().trim().equalsIgnoreCase("") || tv_longitude.getText().toString().trim().equalsIgnoreCase("")) {

			Toast.makeText(mContext, "Please provide your Location.", Toast.LENGTH_SHORT).show();
	...
	private void fetchSurfaceLandAcquisition(String schemeId) {
		...
		volleyTaskManager.doGetSurfaceLandAcquisition(value);
	}
	...
	private void checkingLocation() {
		...
		if (!isGooglePlayServicesAvailable()) {
			...
			Dialog dialog = GooglePlayServicesUtil.getErrorDialog(status, SurfaceLandAcquisitionActivity.this, requestCode);
			dialog.show();
		} else {
	...
	protected void onDestroy() {
		...
		Util.trimCache(SurfaceLandAcquisitionActivity.this);
	}
