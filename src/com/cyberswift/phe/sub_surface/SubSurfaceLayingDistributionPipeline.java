...
public class SubSurfaceLayingDistributionPipeline extends BaseActivity implements ServerResponseCallback, OnClickListener, LocationListener,
...
setContentView(R.layout.subsurface_laying_distribution_pipeline);
mContext = SubSurfaceLayingDistributionPipeline.this;
initView();
...
mProgressDialog = new ProgressDialog(SubSurfaceLayingDistributionPipeline.this);
...
commissioningExpectedTrialRunDateListener = new OnDateSetListener() {
	...
	public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
		Calendar newDate = Calendar.getInstance();
		newDate.set(year, monthOfYear, dayOfMonth);
		tvExpectedDateOfCommisioningOfTheScheme.setText(new SimpleDateFormat("dd/MM/yyyy").format(newDate.getTime()));
	}
};
...
if (!isGooglePlayServicesAvailable()) {
	...
	Dialog dialog = GooglePlayServicesUtil.getErrorDialog(status, SubSurfaceLayingDistributionPipeline.this, requestCode);
	dialog.show();
} else {
...
Util.trimCache(SubSurfaceLayingDistributionPipeline.this);
}
